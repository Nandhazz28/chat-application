...
const MessageBubble = ({
  ...
  const myReaction = Array.isArray(message?.reactions)
    ? message.reactions.find((r) => String(r.userId) === String(currentUserId))
        ?.emoji
    : null;

  return (
    <div
      className={`group flex w-full mb-1 px-4 ${
    ...
    >
      ...
      <div className="relative max-w-[70%]">
        {message?.replyTo && (
          <div
            className={`mb-1 px-3 py-1.5 rounded-xl text-[11px] border-l-2 border-purple-400 bg-white/[0.03] text-slate-400 font-mono truncate ${
              isMine ? "text-right" : ""
            }`}
          >
            <span className="text-purple-400 font-semibold">
              ↩ {message.replyTo?.senderId?.username || "reply"}
            </span>
        ...
        >
          ...
          >
            {message?.edited && (
              <span className="text-[9px] opacity-50 font-mono italic">
                edited
              </span>
            )}
          ...
          >
            {reactionEntries.map(([emoji, count]) => (
              <button
                ...
                className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-full border text-[11px] hover:bg-white/[0.1] transition-colors ${
                  myReaction === emoji
            ...
            <div ref={emojiRef} className="relative">
              ...
              {showEmoji && (
                <div
                  ...
                  } bg-[#0f0a2e] border border-white/10 rounded-2xl p-2 flex gap-1 shadow-2xl`}
                >
                  {EMOJIS.map((e) => (
                    <button
                      key={e}
            ...
            <div ref={menuRef} className="relative">
              ...
              {showMenu && (
                <div
                  className={`absolute z-50 bottom-full mb-1 ${
                    isMine ? "right-0" : "left-0"
                  } bg-[#0f0a2e] border border-white/10 rounded-2xl shadow-2xl overflow-hidden min-w-[150px]`}
                >
                  <button
                    onClick={() => {
